...
{ 
  ...
  {
    int ino = getino2(pathname, &dev);
    printf("Dev:%d Ino:%d\n", dev, ino);
    MINODE *mip = iget(dev, ino);
    /*if(mip->mounted == 1) //moves into dev
    {
...
void ls_file(int ino, char *name, int dev)
{
...
int rpwd(MINODE *wd)
{
  ...
  char buf2[BLKSIZE], myName2[256], myName3[256];
  dev = wd->dev;
  ...
  while(cp < buf + BLKSIZE){
    ...
    { 
      ...
      if(pip->ino == myIno)
      {
        ...
        {
          if (pip->dev == mtable[j].dev)
          {
            pip = mtable[j].mntDirPtr;
            strcpy(myName3, "");
            break;
          }
        }
...
int myrmdir()
{
  ...
  int ino = getino2(pathname, &dev), pino = 0;
...
int myStat()
{
  ...
  if(S_ISDIR(ip->i_mode))
    ...
    printf("link\n");
  printf("Device: %s\t\tInode: %d\t\tLniks: %d\n", mip->mptr->devName, mip->ino, ip->i_links_count);
  printf("Access: (" );
  if (S_ISDIR(ip->i_mode))
    putchar('d');
  else if (S_ISREG(ip->i_mode))
...
int myTouch()
{
  ...
  if(ino == -1) //checks if file exists if doesnt, makes one
  {
    ...
    int devt;
    int pino = getino2(mydirname, &devt);
    if (pino == -1)
      {
        printf("ERROR: such a path doesnt exist(%s)\n", mydirname);
        return -1;
      }
    MINODE *mip = iget(devt, pino);
    INODE *ip = &mip->INODE;
